...
const { ensureStaticData } = require('./sde-updater');

class StaticData {
  constructor() {
    this.staticDataPath = path.join(__dirname, '../staticdata');
    // Auto-update the SDE on startup unless explicitly disabled.
    this.autoUpdate = process.env.SDE_AUTO_UPDATE !== 'false';
  ...
  /**
   * Localized name helper for the new SDE format where `name` is an object
   * keyed by language (e.g. { en: "Rifter", de: "Rifter" }). Falls back to a
   * plain string for forwards/backwards compatibility.
   */
  static localizedName(value) {
  ...
  /**
   * Stream a JSON Lines file into a Map keyed by each record's `_key`.
   * Streaming keeps memory bounded for the large files (types.jsonl ~150MB).
   */
  async _loadJsonl(fileName, map, transform) {
    const filePath = path.join(this.staticDataPath, fileName);
    if (!(await fs.pathExists(filePath))) {
      log.warn(`SDE file not found: ${fileName}`);
      return;
    }
    ...
    for await (const line of rl) {
      ...
      try {
        ...
        log.warn(`Skipping malformed line in ${fileName}`, { error: error.message });
        continue;
      }
      if (record._key === undefined) continue;
      if (transform) transform(record);
      map.set(record._key, record);
    }
  }
  ...
  async loadStaticData() {
    ...
    try {
      // Make sure the JSON Lines SDE is present and up to date before loading.
      await ensureStaticData({ staticDataPath: this.staticDataPath, autoUpdate: this.autoUpdate });

      log.debug('Loading EVE static data files (JSON Lines)...');

      // Types, groups and categories (names are localized objects in the new SDE).
      await this._loadJsonl('types.jsonl', this.typesData);
      await this._loadJsonl('groups.jsonl', this.groupsData);
      await this._loadJsonl('categories.jsonl', this.categoriesData);

      // Per-type dogma. The new SDE marks default effects with a boolean; we
      // normalize it to 0/1 to keep the legacy effect-processing logic intact.
      await this._loadJsonl('typeDogma.jsonl', this.typeDogmaData, (record) => {
        if (Array.isArray(record.dogmaEffects)) {
          for (const effect of record.dogmaEffects) {
            effect.isDefault = effect.isDefault ? 1 : 0;
          }
        }
      });

      // Global dogma attribute definitions.
      await this._loadJsonl('dogmaAttributes.jsonl', this.dogmaAttributesData);

      // Global dogma effect definitions. The `effectName` field was renamed to
      // `name` in the new SDE; alias it back so downstream code is unchanged.
      await this._loadJsonl('dogmaEffects.jsonl', this.dogmaEffectsData, (record) => {
        if (record.effectName === undefined && typeof record.name === 'string') {
          record.effectName = record.name;
        }
      });

      // Populate itemNamesMap for quick case-insensitive lookups.
      for (const [typeId, typeData] of this.typesData.entries()) {
        const typeName = StaticData.localizedName(typeData.name);
        if (typeName) {
  ...
  async getItemInfo(typeId) {
    ...
    return {
      ...
      name: StaticData.localizedName(typeData.name) || `Type ${typeId}`,
      description: StaticData.localizedName(typeData.description) || '',
      group_id: typeData.groupID,
      groupID: typeData.groupID, // Add this for compatibility
      group_name: StaticData.localizedName(groupData?.name) || '',
  ...
  getCategory(categoryId) {
    return this.categoriesData.get(categoryId);
  }
      ...
      if (groupData.categoryID === categoryId) {
        for (const [typeId, typeData] of this.typesData.entries()) {
          if (typeData.groupID === groupId && typeData.published !== false) {
            items.push({
              typeId: typeId,
              name: StaticData.localizedName(typeData.name),
              groupId: groupId,
  ...
  async getShipTypes() {
    ...
    for (const [groupId, groupData] of this.groupsData.entries()) {
      if (groupData.categoryID === 6) { // Ship category
        const groupName = StaticData.localizedName(groupData.name);

        // Get all ships in this group
        const shipsInGroup = [];
        for (const [typeId, typeData] of this.typesData.entries()) {
          if (typeData.groupID === groupId && typeData.published !== false) {
            const shipName = StaticData.localizedName(typeData.name);
            if (shipName) {
              shipsInGroup.push({
                typeId: typeId,
                name: shipName,
