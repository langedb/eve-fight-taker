...
class ESIAuth {
  constructor(options = {}) {
    ...
    if (!options.redirectUri.match(/^https?:\/\//)) {
    ...
    this.generatedState = null; // Store generated state for validation
  }

  getAuthorizationUrl() {
    const state = crypto.randomBytes(16).toString('hex');
    this.generatedState = state; // Store for later validation
  ...
  async exchangeCodeForTokens(code) {
    ...
    const response = await rateLimitHandler.executeRequest(() =>
      axios.post(`${this.loginUrl}/v2/oauth/token`,
        new URLSearchParams({
  ...
  async getCharacterInfo(accessToken) {
    ...
    const verifyResponse = await rateLimitHandler.executeRequest(() =>
      axios.get(`${this.loginUrl}/oauth/verify`, {
    ...
    const characterResponse = await rateLimitHandler.executeRequest(() =>
      axios.get(`${this.baseUrl}/characters/${characterId}/`, {
        headers: { 'Authorization': `Bearer ${accessToken}` }
      })
    );
  ...
  async getCurrentShip(accessToken) {
    ...
    const response = await rateLimitHandler.executeRequest(() =>
      axios.get(`${this.baseUrl}/characters/${character.id}/ship/`, {
        headers: { 'Authorization': `Bearer ${accessToken}` }
      })
    );
  ...
  async getCurrentShipFit(accessToken) {
    ...
    try {
      ...
      log.error('Error getting current ship fit', { error: error.message, stack: error.stack });
      throw error;
    }
  }
  ...
  async getShipFitting(accessToken, shipTypeId) {
    ...
    const fittingsResponse = await rateLimitHandler.executeRequest(() =>
      axios.get(`${this.baseUrl}/characters/${character.id}/fittings/`, {
        headers: { 'Authorization': `Bearer ${accessToken}` }
      })
    );
  ...
  async getItemInfo(itemId) {
    const response = await rateLimitHandler.executeRequest(() =>
      axios.get(`${this.baseUrl}/universe/types/${itemId}/`)
    );
    return response.data;
  }
}
