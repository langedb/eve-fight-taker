...
class ZKillboardParser {
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (slotType && slots[slotType] !== undefined) {
          // Handle charges (ammo) - they should be associated with their weapon
          if (this.isCharge(item.flag)) {
            // Find the weapon this charge belongs to and add it
            const weaponIndex = slots.high.length - 1;
            if (weaponIndex >= 0 && !slots.high[weaponIndex].includes(',')) {
              slots.high[weaponIndex] += `, ${itemName}`;
            }
          } else if (slotType === 'drones' && quantity > 1) {
      ...
      // High slots
      for (const module of slots.high) {
        eftText += `${module}\n`;
      }
      eftText += '\n';

      // Mid slots  
      for (const module of slots.med) {
        eftText += `${module}\n`;
      }
      eftText += '\n';

      // Low slots
      for (const module of slots.low) {
        eftText += `${module}\n`;
      }
      eftText += '\n';

      // Rigs
      for (const rig of slots.rig) {
        eftText += `${rig}\n`;
      }
      eftText += '\n';

      // Subsystems
      for (const subsystem of slots.subsystem) {
        eftText += `${subsystem}\n`;
      }
      if (slots.subsystem.length > 0) eftText += '\n';

      // Drones
      for (const drone of slots.drones) {
        eftText += `${drone}\n`;
      }
      if (slots.drones.length > 0) eftText += '\n';
  ...
  getSlotTypeFromFlag(flag) {
    ...
    if (flag === 87 || flag === 88) return 'drones'; // Drone bay
