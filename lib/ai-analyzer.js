...
2. AMMO RECOMMENDATIONS: Recommend specific ammo types based on target's tank type and resistances
...
Provide tactical analysis in this JSON format:
{
  ...
  "ammoRecommendations": ["specific ammo type recommendations based on fitted weapons"],
...
Be specific about actual fitted modules, weapon types, optimal ranges, and ammo choices. This is a real PvP analysis, not generic advice.`;
  ...
  async getDamageInfo(itemInfo) {
    ...
    // Override for known drone damage types that may be incorrect in static data
    ...
    for (const attr of itemInfo.attributes) {
      switch (attr.attributeID) {
        ...
        case 116: damages.thermal = attr.value || 0; break;
        case 117: damages.kinetic = attr.value || 0; break;
        case 118: damages.explosive = attr.value || 0; break;
      }
    }
