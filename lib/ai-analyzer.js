...
CRITICAL TRIGLAVIAN ENTROPIC DISINTEGRATOR SPOOL MECHANICS:
- **TRIGLAVIAN SHIPS** (Damavik, Vedmak, Leshak, Kikimora, Zarmazd) use Entropic Disintegrators with PROGRESSIVE DAMAGE RAMP-UP
- **SPOOL MECHANIC**: Damage increases over 4 consecutive cycles when firing at the SAME target:
  * Cycle 1: 100% damage (BASE DPS)
  * Cycle 2: 150% damage
  * Cycle 3: 200% damage
  * Cycle 4+: 250% damage (MAX SPOOL DPS - sustained performance)
- **TARGET SWITCHING RESETS SPOOL**: Changing targets resets damage back to 100% (Cycle 1)
- **TACTICAL IMPLICATIONS**:
  * **Your Ship is Triglavian**: START firing early to build spool, maintain fire on primary target, avoid target switching
  * **Enemy is Triglavian**: DISRUPT their spool by forcing them to switch targets (ECM, breaking tackle) or kill them before max spool
  * **Time to Max DPS**: Requires 3-4 weapon cycles (~12-20 seconds typically) to reach maximum damage output
  * **DPS Values in Analysis**: "Max Spool DPS" = sustained damage after ramp-up, "Base DPS" = initial damage before spool
- **WEAPON SYSTEMS**: Light Entropic Disintegrator (frigates/destroyers), Heavy Entropic Disintegrator (cruisers), Supratidal Entropic Disintegrator (dreadnoughts)
- **NO FALLOFF RANGE**: Entropic disintegrators have optimal range but NO falloff - damage drops critically beyond optimal
- **DAMAGE TYPES**: Mix of thermal and explosive damage (varies by ammo: Baryon, Meson, Tetryon)
- **ENGAGEMENT STRATEGY**:
  * **Against Triglavian**: Engage before they fully spool, use high alpha/burst damage, or force target switching
  * **With Triglavian**: Prioritize targets, commit to kills, use spool time to position optimally
  * **Fleet Context**: Triglavian ships excel in sustained engagements against single high-value targets (capitals, battleships)
...
CRITICAL COMBAT BOOSTER EFFECTS REFERENCE:

**BLUE PILL BOOSTER** (Shield Boosting Enhancement):
- **PRIMARY EFFECT**: Increases shield booster amount by 20-30% (Standard/Improved/Strong grades)
- **DOES NOT** affect capacitor positively - capacitor capacity penalty is a negative side effect
- **CORRECT USAGE**: "Use Blue Pill to enhance your shield booster effectiveness" 
- **INCORRECT USAGE**: "Use Blue Pill to enhance capacitor" (THIS IS WRONG)
- **MECHANISM**: Makes shield booster modules repair more shield HP per cycle
- **TACTICAL APPLICATION**: Most effective with active shield tanking ships that rely on shield boosters

**EXILE BOOSTER** (Armor Repair Enhancement):
- **PRIMARY EFFECT**: Increases armor repairer amount by 20-30%
- **MECHANISM**: Makes armor repair modules repair more armor HP per cycle
- **TACTICAL APPLICATION**: Most effective with active armor tanking ships

**MINDFLOOD BOOSTER** (Capacitor Enhancement):
- **PRIMARY EFFECT**: Increases capacitor capacity by 20-25%
- **MECHANISM**: Provides more total capacitor pool for sustained module operation
- **TACTICAL APPLICATION**: Extends engagement time for capacitor-hungry fits

**COMBAT BOOSTER TACTICAL GUIDELINES**:
- Only recommend boosters that are actually present in the ship's cargo
- Match booster type to ship's tanking strategy (Blue Pill for shield tanks, Exile for armor tanks)
- Blue Pill is ONLY for shield boosting enhancement, NOT capacitor enhancement
- Explain the correct mechanism when recommending booster usage
- Consider booster duration (30 minutes base) for engagement timing
...
Be specific about actual fitted modules, weapon types, optimal ranges, and ammo choices for YOUR SHIP. This is a real PvP analysis, not generic advice.`;
  ...
  async getModuleClassification(moduleInfo) {
    ...
    if (moduleInfo.group_id) {
      ...
      await this.ensureStaticData();
      if (this.staticData.isWeaponModule(moduleInfo.group_id)) {
        return {
          type: 'active',
  ...
  formatCalculatedWeaponStats(stats) {
    ...
    for (const weapon of stats.weapons) {
      ...
      // Add DPS with Triglavian spool info if applicable
      if (weapon.triglavianSpool) {
        // This is a Triglavian entropic disintegrator with spool mechanics
        weaponText += ' → 🔷 TRIGLAVIAN SPOOL WEAPON:';
        weaponText += `\n  • Base DPS (Cycle 1): ${weapon.triglavianSpool.baseDPS.total.toFixed(1)}`;
        weaponText += `\n  • Average DPS (Cycles 1-4): ${weapon.triglavianSpool.averageDPS.total.toFixed(1)}`;
        weaponText += `\n  • Max Spool DPS (Cycle 4+): ${weapon.triglavianSpool.maxSpoolDPS.total.toFixed(1)}`;
        weaponText += `\n  • Spool Progression: ${weapon.triglavianSpool.spoolMultipliers.map((m, i) => `Cycle ${i+1}: ${(m*100).toFixed(0)}%`).join(', ')}`;
        weaponText += `\n  ⚠️ CRITICAL: Damage ramps up over ${weapon.triglavianSpool.maxSpoolCycles} cycles. Target switching resets spool to base damage!`;
      } else if (weapon.dps !== undefined && typeof weapon.dps === 'number') {
  ...
  async analyzeRigEffects(fit, stats, _staticData) {
    ...
    if (rigsByType.hydraulicBayThrusters.length > 0) {
      ...
      for (let i = 0; i < rigCount; i++) {
        await this.ensureStaticData();
        const stackingPenalties = this.staticData.getStackingPenalties();
        const stackingPenalty = stackingPenalties[Math.min(i, 4)];
  ...
  /**
   * Generic method for analyzing with custom prompts (used by fleet analyzer)
   * @param {string} prompt - The analysis prompt
   * @returns {string} - AI response
   */
  async analyzeWithPrompt(prompt) {
    try {
      log.debug('Starting Gemini AI analysis with new prompt method...');

      // Corrected implementation using generateContent directly from the model
      const response = await this.genAI.models.generateContent({
        model: this.model,
        contents: prompt
      });
      ...
      log.debug('Gemini AI analysis completed successfully');
      log.debug('Response length:', text.length);

      // Defensive check for empty or invalid response
      if (!text || text.trim() === '') {
        log.warn('Gemini AI returned an empty response.');
      ...
      // Re-throw a more informative error to be caught by the fleet analyzer
