...
const { calculateModuleTank } = require('./tank-calculator');
...
class FitCalculator {
  ...
  // Check if item is a module/weapon by category
  isModuleItem(itemInfo) {
    // Use category-based detection instead of hard-coded group arrays
    return itemInfo.category_id === 7; // Category 7 = Module
  }

  // Check if item is a charge/script/ammo by category
  isChargeItem(itemInfo) {
    // Use category-based detection instead of hard-coded group arrays
    return itemInfo.category_id === 8; // Category 8 = Charge
  }
  ...
  async parseEFT(eftText) {
    ...
    const shipType = firstLine.substring(firstBracket + 1, comma).trim();
    const fitName = firstLine.substring(comma + 1, secondBracket).trim();

    const fit = {
      shipType,
      shipName: shipType,  // Add shipName for compatibility with FitSimulator
      fitName,
      modules: {
        high: [],
        med: [],
    ...
    let moduleSlot = 'low';  // EFT format starts with low slots (official specification)
    let hasModulesInCurrentSlot = false; // Track if we've seen modules in current slot
    let previousWasEmpty = false; // Track if previous line was empty
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (line === '') {
        ...
        // EFT format order: Low → Med → High → Rigs → Subsystems → Drones → Cargo (official specification)
        if (currentSection === 'modules') {
          // Advance to next slot if:
          // 1. We have modules in current slot, OR
          // 2. Previous line was NOT empty (first empty line in a sequence)
          // This ensures we advance once per section, not on every empty line
          if (hasModulesInCurrentSlot || !previousWasEmpty) {
            ...
            hasModulesInCurrentSlot = false; // Reset for next slot
          }
      ...
      // We're processing a non-empty line
      previousWasEmpty = false;

      if (currentSection === 'modules') {
        const isRigOrSubsystemSlot = moduleSlot === 'rig' || moduleSlot === 'subsystem';
        // Check if line is ammo - exclude "launcher" to avoid matching "Missile Launcher" as ammo
        const lineLower = line.toLowerCase();
        const isLauncher = lineLower.includes('launcher');
        const isAmmo = !isLauncher && ['charge', 'missile', 'rocket', 'torpedo', 'condenser pack', 'crystal', 'ammo', 'electropunch', 'snipestrike', 'blastshot', 'galvasurge', 'slambolt'].some(keyword => lineLower.includes(keyword));

        // When in rig or subsystem slots, if we see ammo/cargo, switch to cargo section
        if (isRigOrSubsystemSlot && isAmmo) {
          currentSection = 'cargo';
          const cargoInfo = await this.parseCargoLine(line);
          if (cargoInfo) {
        ...
        // Skip empty slots but mark that we've seen content in this slot
        if (!line || line.toLowerCase().includes('[empty')) {
          hasModulesInCurrentSlot = true;  // Mark slot as "processed" so next empty line will advance
          continue;
        }
        ...
        if (itemInfo) {
          ...
          // Check if we're in rig/subsystem slot but item is a charge (category 8) - treat as cargo
          else if (isRigOrSubsystemSlot && itemInfo.category_id === 8) {
            // Charge item in rig/subsystem slot, must be cargo - switch to cargo section
            currentSection = 'cargo';
            const cargoInfo = await this.parseCargoLine(line);
            if (cargoInfo) {
              fit.cargo.push(cargoInfo);
          ...
          // Check if we're in subsystem slot but item is not a subsystem (category 32)
          else if (moduleSlot === 'subsystem' && itemInfo.category_id !== 32) {
            // Not a subsystem, must be cargo - switch to cargo section
            currentSection = 'cargo';
            const cargoInfo = await this.parseCargoLine(line);
            if (cargoInfo) {
              fit.cargo.push(cargoInfo);
            ...
            if (moduleInfo) {
              ...
              hasModulesInCurrentSlot = true;
            }
          }
        } else {
          // Fallback to keyword detection if static data lookup fails
          // TODO: Could be enhanced to use comprehensive static data search
          ...
          // Check if we're in rig/subsystem slot but couldn't find a module match
          else if (isRigOrSubsystemSlot) {
            // Probably cargo - switch to cargo section
            currentSection = 'cargo';
            const cargoInfo = await this.parseCargoLine(line);
            if (cargoInfo) {
              fit.cargo.push(cargoInfo);
        ...
        // Skip empty lines in drones section
        if (!line || line.toLowerCase().includes('[empty')) {
          continue;
        }
    ...
    // Post-processing: Move T3 subsystems from rig to subsystem section
    const t3Subsystems = [];
    fit.modules.rig = fit.modules.rig.filter(module => {
      // Check if this is a T3 subsystem (contains ship name and typical subsystem names)
      const isT3Subsystem = /^(Loki|Tengu|Proteus|Legion)\s+(Defensive|Offensive|Propulsion|Engineering)\s+-/.test(module.name);
      if (isT3Subsystem) {
        t3Subsystems.push(module);
        return false; // Remove from rig
      }
      return true; // Keep in rig
    });

    // Add T3 subsystems to subsystem section
    fit.modules.subsystem.push(...t3Subsystems);
  ...
  async esiToEFT(esiFitting) {
    ...
    // Group items by flag to handle loaded charges properly
    const itemsByFlag = new Map();
    for (const item of esiFitting.items) {
      const flag = item.flag;
      if (!itemsByFlag.has(flag)) {
        itemsByFlag.set(flag, []);
      }
      itemsByFlag.get(flag).push(item);
    }

    // Process each flag group
    for (const [flag, flagItems] of itemsByFlag) {
      const flagStr = String(flag);
      let slotType = null;

      // Determine slot type
      if (flagStr.startsWith('LoSlot') || (flag >= 11 && flag <= 18)) {
        slotType = 'low';
      } else if (flagStr.startsWith('MedSlot') || (flag >= 19 && flag <= 26)) {
        slotType = 'med';
      } else if (flagStr.startsWith('HiSlot') || (flag >= 27 && flag <= 34)) {
        slotType = 'high';
      } else if (flagStr.startsWith('RigSlot') || (flag >= 92 && flag <= 99)) {
        slotType = 'rig';
      } else if (flagStr.startsWith('SubSystem') || (flag >= 134 && flag <= 143)) {
        slotType = 'subsystem';
      } else if (flagStr === 'DroneBay' || flag === 87) {
        slotType = 'drones';
      } else if (flagStr === 'Cargo' || flag === 5) {
        slotType = 'cargo';
      }

      if (slotType === 'high' || slotType === 'med' || slotType === 'low') {
        // For fitting slots, identify weapon/module vs charge
        let module = null;
        let charge = null;
        ...
        for (const item of flagItems) {
          ...
          if (!itemInfo) {
            log.warn(`Unknown item type_id: ${item.type_id}`);
            continue;
          }

          // Check if this is a module or charge based on category and group
          if (this.isModuleItem(itemInfo)) {
            module = { item, itemInfo };
          } else if (this.isChargeItem(itemInfo)) {
            charge = { item, itemInfo };
          } else {
            // Default to module if unsure
            module = { item, itemInfo };
          }
        }
        ...
        // Add to EFT sections
        if (module) {
          const entryName = charge ? `${module.itemInfo.name}, ${charge.itemInfo.name}` : module.itemInfo.name;
          for (let i = 0; i < module.item.quantity; i++) {
            eftSections[slotType].push(entryName);
          }
        }
      } else if (slotType === 'drones' || slotType === 'cargo') {
        for (const item of flagItems) {
          const itemInfo = await this.staticData.getItemInfo(item.type_id);
          if (!itemInfo) {
            log.warn(`Unknown item type_id: ${item.type_id}`);
          ...
          eftSections[slotType].push(`${itemInfo.name}${item.quantity > 1 ? ' x' + item.quantity : ''}`);
        }
      } else if (slotType === 'rig' || slotType === 'subsystem') {
        for (const item of flagItems) {
          const itemInfo = await this.staticData.getItemInfo(item.type_id);
          if (!itemInfo) {
            log.warn(`Unknown item type_id: ${item.type_id}`);
          ...
          for (let i = 0; i < item.quantity; i++) {
            eftSections[slotType].push(itemInfo.name);
          }
        }
      } else {
        // Handle unmapped flags - add to cargo
        for (const item of flagItems) {
          const itemInfo = await this.staticData.getItemInfo(item.type_id);
          if (!itemInfo) {
            log.warn(`Unknown item type_id: ${item.type_id}`);
          ...
          if (itemInfo.category_id === 6) {
            // Modules go without quantity
            for (let i = 0; i < item.quantity; i++) {
              eftSections.cargo.push(itemInfo.name);
            }
          } else {
            // Other items go with quantity
    ...
    // Official EFT section order: Low → Med → High → Rigs → Subsystems → Drones → Cargo (official specification)
    const sections = [
  ...
  async parseCargoLine(line) {
    const match = line.match(/^(.+?)\s+x(\d+)$/);
    if (match) {
      const name = match[1].trim();
      const itemInfo = await this.getItemByName(name);

      return {
        name: name,
        quantity: parseInt(match[2]),
        category_id: itemInfo?.category_id,
        group_id: itemInfo?.group_id
      };
    } else if (line.trim()) {
      // Handle items without quantities (default to 1)
  ...
  isEntropicDisintegrator(moduleInfo) {
    if (!moduleInfo) return false;
    return moduleInfo.group_id === 1986; // Triglavian Entropic Disintegrators
  }

  calculateTriglavianSpool(baseDamage, cycleTime) {
    // Triglavian entropic disintegrators have a spool mechanic:
    // Cycle 1: 100% damage
    // Cycle 2: 150% damage
    // Cycle 3: 200% damage
    // Cycle 4+: 250% damage (max spool)

    const spoolMultipliers = [1.0, 1.5, 2.0, 2.5];
    const maxSpoolCycles = 4;

    // Calculate DPS for each spool stage
    const dpsFactor = (1000 / cycleTime);
    const spoolStages = spoolMultipliers.map(mult => baseDamage * mult * dpsFactor);

    // Calculate average DPS over the spool-up period (first 4 cycles)
    const averageDamagePerCycle = (baseDamage * 1.0 + baseDamage * 1.5 + baseDamage * 2.0 + baseDamage * 2.5) / maxSpoolCycles;
    const averageDPS = averageDamagePerCycle * dpsFactor;

    return {
      baseDPS: spoolStages[0],           // Cycle 1: 100%
      maxSpoolDPS: spoolStages[3],       // Cycle 4+: 250%
      averageDPS: averageDPS,             // Average over 4 cycles
      spoolStages: spoolStages,           // All 4 stages
      spoolMultipliers: spoolMultipliers, // Multipliers for each stage
      maxSpoolCycles: maxSpoolCycles      // Number of cycles to reach max
    };
  }

  isMissileLauncher(moduleInfo) {
    if (!moduleInfo) return false;

    // Missile launcher group IDs from EVE static data
    const missileGroups = [
      56,   // Missile Launcher (basic)
      506,  // Missile Launcher Cruise
      507,  // Missile Launcher Rocket
      508,  // Missile Launcher Torpedo
      509,  // Missile Launcher Light
    ...
    return missileGroups.includes(moduleInfo.group_id);
  }

  async findOptimalAmmoFromCargo(weaponInfo, fitSimulator) {
    const fit = fitSimulator.fit;

    if (!fit.cargo || fit.cargo.length === 0) {
      return null;
    }
  ...
  isAmmoCompatibleWithWeapon(weaponInfo, ammoInfo) {
    ...
    if (weaponSize !== ammoSize) {
      return false;
    }

    // Type compatibility using generic static data method
    const compatibility = this.staticData.getWeaponAmmoCompatibility();

    // Check each weapon type
    for (const weaponType of Object.keys(compatibility)) {
      const config = compatibility[weaponType];
      if (config.weaponGroups.includes(weaponInfo.group_id) &&
          config.ammoGroups.includes(ammoInfo.group_id)) {
        return true;
      }
    }

    // Fallback to name-based matching for edge cases
  ...
  extractWeaponSize(weaponName) {
    ...
    // Special handling for missile launchers which use descriptive names
    if (weaponName.includes('light missile launcher') || weaponName.includes('rocket launcher')) return 'small';
    if (weaponName.includes('heavy missile launcher') || weaponName.includes('heavy assault missile launcher') || weaponName.includes('rapid heavy missile launcher')) return 'medium';
    if (weaponName.includes('cruise missile launcher') || weaponName.includes('torpedo launcher')) return 'large';
    if (weaponName.includes('citadel launcher')) return 'capital';
    ...
    // Medium weapons: 220mm-350mm
    if (weaponName.includes('220mm') || weaponName.includes('250mm') ||
        weaponName.includes('280mm') || weaponName.includes('350mm')) return 'medium';

    // Large weapons: 425mm+
    if (weaponName.includes('425mm') || weaponName.includes('650mm') ||
        weaponName.includes('720mm') || weaponName.includes('800mm') ||
        weaponName.includes('1400mm')) return 'large';
  ...
  extractAmmoSize(ammoName) {
    ...
    if (ammoName.includes('light missile') || ammoName.includes('light rocket') || ammoName.includes(' rocket')) return 'small';
  ...
  async calculateFitStats(fit) {
    ...
    const stats = {
      ...
      weapons: [] // Array of weapon details including velocity, range, etc.
    };

    let totalGlobalDamageBonus = 0; // Accumulate global damage bonuses from modules

    // Get ship base stats
    const shipInfo = await this.getItemByName(fit.shipType || fit.shipName);
    if (!shipInfo) {

      return this.generateMockFitStats(fit.shipType || fit.shipName);
    }


    // Normalize fit object for FitSimulator compatibility
    const normalizedFit = { ...fit };
    if (fit.shipType && !fit.shipName) {
      normalizedFit.shipName = fit.shipType;
    }
    // Also ensure shipType is available
    if (fit.shipName && !fit.shipType) {
      normalizedFit.shipType = fit.shipName;
    }

    const fitSimulator = new FitSimulator(normalizedFit, this.staticData);
    ...
    // Third pass: Calculate active tank (shield boosters, armor repairers)
    for (const slotType of ['high', 'med', 'low']) {
      for (const module of fit.modules[slotType]) {
        if (!module.offline) {
          const moduleInfo = await this.getItemByName(module.name);
          if (moduleInfo) {
            const moduleTank = await calculateModuleTank(module, moduleInfo, fitSimulator);
            stats.tank.shield += moduleTank.shield;
            stats.tank.armor += moduleTank.armor;
          }
        }
      }
    }
    stats.tank.total = stats.tank.shield + stats.tank.armor + stats.tank.hull;
  ...
  async getItemByName(itemName) {
    ...
    try {
      if (!itemName) {
        log.error('getItemByName called with undefined or null itemName');
        return null;
      }
      ...
      log.error(`Error searching for item ${itemName}`, { error: error.message });
      return null;
    }
  }
  ...
  async calculateShipBaseStats(shipInfo, stats, fitSimulator = null) {
    ...
    if (maxVelocity !== null) {
      // Apply Navigation skill bonus: 5% per level = 25% at level V
      stats.speed = maxVelocity * 1.25;
    }
    ...
    if (scanResolution !== null) {
      // Apply Signature Analysis skill bonus: 5% per level = 25% at level V
      stats.scanResolution = scanResolution * 1.25;
    }
  ...
  async calculateEHP(stats, fit, fitSimulator) {
    ...
    const shipInfo = await this.staticData.searchItemByName(fit.shipName || fit.shipType);
    if (!shipInfo) return;
    ...
    const shieldResistances = {
      ...
      thermal: await fitSimulator.getModifiedAttribute(shipInfo.name, 274), // shieldThermalDamageResonance
    ...
    // Calculate average resonance for each layer (resonance = damage taken)
    // In EVE, resonance values like 0.2 mean 20% damage taken (80% resistance)
    // EHP = HP / average_resonance_across_damage_types

    const calculateAverageResonance = (resistances) => {
      let averageResonance = 0;
      for (const type of ['em', 'thermal', 'kinetic', 'explosive']) {
        // Resistances here are actually resonance values (damage multipliers)
        // Handle null/undefined values as 1.0 (no resistance)
        const resonance = resistances[type] || 1.0;
        averageResonance += damageDistribution[type] * resonance;
      }
      // Ensure minimum resonance to avoid division by zero or negative values
      return Math.max(averageResonance, 0.01);
    };

    const averageShieldResonance = calculateAverageResonance(shieldResistances);
    const averageArmorResonance = calculateAverageResonance(armorResistances);
    const averageHullResonance = calculateAverageResonance(hullResistances);

    // Store resistances in stats (convert resonance to resistance % for display)
    stats.shieldResistances = {
      em: Math.max(0, Math.min(100, (1 - shieldResistances.em) * 100)),
      thermal: Math.max(0, Math.min(100, (1 - shieldResistances.thermal) * 100)),
      kinetic: Math.max(0, Math.min(100, (1 - shieldResistances.kinetic) * 100)),
      explosive: Math.max(0, Math.min(100, (1 - shieldResistances.explosive) * 100))
    };
    stats.armorResistances = {
      em: Math.max(0, Math.min(100, (1 - armorResistances.em) * 100)),
      thermal: Math.max(0, Math.min(100, (1 - armorResistances.thermal) * 100)),
      kinetic: Math.max(0, Math.min(100, (1 - armorResistances.kinetic) * 100)),
      explosive: Math.max(0, Math.min(100, (1 - armorResistances.explosive) * 100))
    };
    stats.hullResistances = {
      em: Math.max(0, Math.min(100, (1 - hullResistances.em) * 100)),
      thermal: Math.max(0, Math.min(100, (1 - hullResistances.thermal) * 100)),
      kinetic: Math.max(0, Math.min(100, (1 - hullResistances.kinetic) * 100)),
      explosive: Math.max(0, Math.min(100, (1 - hullResistances.explosive) * 100))
    };

    // Calculate EHP for each layer: EHP = HP / average_resonance
    const modifiedShieldCapacity = await fitSimulator.getModifiedAttribute(shipInfo.name, 263);
    stats.ehp.shield = (modifiedShieldCapacity || 0) / (averageShieldResonance || 1);
    stats.ehp.armor = (stats.ehp.armor || 0) / (averageArmorResonance || 1);
    stats.ehp.hull = (stats.ehp.hull || 0) / (averageHullResonance || 1);
  ...
  async applyModuleStats(module, stats, _shipInfo = null, _globalDamageBonus = 0, fitSimulator) {
    const moduleInfo = await this.getItemByName(module.name);
    if (!moduleInfo) {
      log.debug(`moduleInfo not found for ${module.name}`);
      return;
    }
    // Debug logging can be removed now that ammo selection is working
    ...
    if (module.charge) {
      ...
      if (optimalAmmo) {
        ...
        // Actually update the module charge so it shows in debug output
        module.charge = optimalAmmo.name;

        // Initialize and apply bonuses to the auto-loaded charge
        await fitSimulator.initializeAndApplyChargeBonus(optimalAmmo.name);
      } else {
    ...
    if (damageSourceItemName) {
      // Get base damage from missile/ammo with all skill bonuses applied
      ...
      // Try to get launcher damage multiplier, default to 1 if not found (for missiles)
    ...
    // console.log(`DEBUG cycle time: module=${module.name}, base=${baseCycleTime}ms, modified=${finalCycleTime}ms`);

    // Calculate DPS and Volley
    let triglavianSpool = null;
    if (emDamage || thermalDamage || kineticDamage || explosiveDamage) {
      const dpsFactor = (1000 / finalCycleTime);
      const totalBaseDamage = emDamage + thermalDamage + kineticDamage + explosiveDamage;

      // Check if this is a Triglavian entropic disintegrator
      if (this.isEntropicDisintegrator(moduleInfo)) {
        // Calculate spool mechanics for Triglavian weapons
        triglavianSpool = this.calculateTriglavianSpool(totalBaseDamage, finalCycleTime);

        // For stats.dps, use max spool DPS (sustained damage after spooling up)
        // This represents the weapon's sustained performance
        const spoolMultiplier = 2.5; // Max spool multiplier

        stats.dps.em += emDamage * dpsFactor * spoolMultiplier;
        stats.dps.thermal += thermalDamage * dpsFactor * spoolMultiplier;
        stats.dps.kinetic += kineticDamage * dpsFactor * spoolMultiplier;
        stats.dps.explosive += explosiveDamage * dpsFactor * spoolMultiplier;

        // Volley damage is still base (first shot)
    ...
    // Collect weapon information if this is a weapon module with damage
    if (this.isWeaponModule(moduleInfo) && damageSourceItemName &&
        (emDamage > 0 || thermalDamage > 0 || kineticDamage > 0 || explosiveDamage > 0)) {

      // Calculate range based on weapon type
      let velocity = 0, flightTime = 0, range = 0, optimalRange = 0, falloff = 0, trackingSpeed = 0;

      if (this.isMissileLauncher(moduleInfo)) {
        // Missile weapons: range = velocity * flight time
        velocity = await fitSimulator.getModifiedAttribute(damageSourceItemName, 37) || 0; // maxVelocity
        flightTime = await fitSimulator.getModifiedAttribute(damageSourceItemName, 281) || 0; // maxFlightTime
        range = velocity * (flightTime / 1000); // Convert flight time from ms to seconds
      } else {
        // Turret weapons: range = optimal + falloff
        optimalRange = await fitSimulator.getModifiedAttribute(module.name, 54) || 0; // maxRange (optimal)
        falloff = await fitSimulator.getModifiedAttribute(module.name, 158) || 0; // falloff
        trackingSpeed = await fitSimulator.getModifiedAttribute(module.name, 160) || 0; // trackingSpeed
        range = optimalRange + falloff; // Total effective range for turrets
      }

      // Get rate of fire for DPS calculation
      const cycleTime = await fitSimulator.getModifiedAttribute(module.name, 51) || 1000; // attributeID 51 for Rate of fire / cycle time
      const rof = 60000 / cycleTime; // Convert to rounds per minute

      // Create weapon info object
      const dpsFactor = (1000 / cycleTime);
      const weaponInfo = {
        name: module.name,
        chargeName: damageSourceItemName !== module.name ? damageSourceItemName : null,
        velocity: velocity,
        ...
        rateOfFire: rof,
        dps: {
          total: (emDamage + thermalDamage + kineticDamage + explosiveDamage) * dpsFactor,
          em: emDamage * dpsFactor,
          thermal: thermalDamage * dpsFactor,
          kinetic: kineticDamage * dpsFactor,
          explosive: explosiveDamage * dpsFactor
        },
        usedAmmoFromCargo: usedAmmoFromCargo
      };

      // Add Triglavian spool mechanics if applicable
      if (triglavianSpool) {
        weaponInfo.triglavianSpool = {
          baseDPS: {
            total: triglavianSpool.baseDPS,
            em: emDamage * dpsFactor,
            thermal: thermalDamage * dpsFactor,
            kinetic: kineticDamage * dpsFactor,
            explosive: explosiveDamage * dpsFactor
          ...
          averageDPS: {
            total: triglavianSpool.averageDPS,
            em: emDamage * dpsFactor * 1.625, // Average multiplier: (1.0+1.5+2.0+2.5)/4 = 1.75
            thermal: thermalDamage * dpsFactor * 1.625,
            kinetic: kineticDamage * dpsFactor * 1.625,
            explosive: explosiveDamage * dpsFactor * 1.625
          },
          maxSpoolDPS: {
            total: triglavianSpool.maxSpoolDPS,
            em: emDamage * dpsFactor * 2.5,
            thermal: thermalDamage * dpsFactor * 2.5,
            kinetic: kineticDamage * dpsFactor * 2.5,
            explosive: explosiveDamage * dpsFactor * 2.5
          },
          spoolStages: triglavianSpool.spoolStages,
          spoolMultipliers: triglavianSpool.spoolMultipliers,
          maxSpoolCycles: triglavianSpool.maxSpoolCycles
        };

        // Override the main DPS values to show max spool
        weaponInfo.dps = weaponInfo.triglavianSpool.maxSpoolDPS;
      }

      stats.weapons.push(weaponInfo);
    }
  }
  ...
  async applyDroneStats(drone, stats, fitSimulator) {
    ...
    const emDamage = await fitSimulator.getModifiedAttribute(droneName, 114) || 0; // EM damage
    const explosiveDamage = await fitSimulator.getModifiedAttribute(droneName, 116) || 0; // Explosive damage
    const kineticDamage = await fitSimulator.getModifiedAttribute(droneName, 117) || 0; // Kinetic damage
    const thermalDamage = await fitSimulator.getModifiedAttribute(droneName, 118) || 0; // Thermal damage

    droneRawDamage = emDamage + explosiveDamage + kineticDamage + thermalDamage;

    // console.log(`DEBUG - Drone ${droneName} damage: EM=${emDamage}, Exp=${explosiveDamage}, Kin=${kineticDamage}, Therm=${thermalDamage}, Total=${droneRawDamage}`);
    ...
    if (!damageBrokenDown) {
      const emDps = await fitSimulator.getModifiedAttribute(droneName, 114) || 0;
      const explosiveDps = await fitSimulator.getModifiedAttribute(droneName, 116) || 0;
      const kineticDps = await fitSimulator.getModifiedAttribute(droneName, 117) || 0;
      const thermalDps = await fitSimulator.getModifiedAttribute(droneName, 118) || 0;
    ...
    // Accumulate total drone DPS for separate tracking
    stats.droneDPS += droneDPS;

    // console.log(`DEBUG - DPS total recalculated in applyDroneStats: ${stats.dps.total} (EM:${stats.dps.em}, Therm:${stats.dps.thermal}, Kin:${stats.dps.kinetic}, Exp:${stats.dps.explosive})`);
  }
  ...
  async applyFighterStats(fighter, stats, fitSimulator) {
    ...
    // Fighter damage fallback values - try static data first, then hard-coded fallback
    if (emDamage === 0 && explosiveDamage === 0 && kineticDamage === 0 && thermalDamage === 0) {
      const fighterStats = await this.staticData.getFighterStats(fighterName);
      if (fighterStats) {
        emDamage = fighterStats.em;
        explosiveDamage = fighterStats.explosive;
        kineticDamage = fighterStats.kinetic;
        thermalDamage = fighterStats.thermal;
      } else {
        // Final fallback to hard-coded database if static data fails
    ...
    const fighterRateOfFire = await fitSimulator.getModifiedAttribute(fighterName, 51) || 2000; // Default 2 seconds for fighters
    const damageBonusMultiplier = await fitSimulator.getModifiedAttribute(fighterName, 64) || 1;
    ...
    // Check if we're using static data vs hard-coded fallback
    const usingStaticData = await this.staticData.getFighterStats(fighterName);
    const isDirectDPS = usingStaticData || this.getFighterBaseDPS(fighterName);
  ...
  async calculateShipStats(fit, fitSimulator) {
    // This method is similar to calculateFitStats but uses an external FitSimulator
    await this.ensureStaticData();
    ...
    let totalGlobalDamageBonus = 0;

    // Get ship base stats
    const shipInfo = await this.getItemByName(fit.shipName || fit.shipType);
    if (!shipInfo) {
      return this.generateMockFitStats(fit.shipName || fit.shipType);
    }
    ...
    // Apply all effects to initialize attributes and bonuses
    await fitSimulator.applyEffects();

    await this.calculateShipBaseStats(shipInfo, stats, fitSimulator);

    // Process modules to find global damage bonuses
    for (const slotType of ['high', 'med', 'low', 'rig', 'subsystem']) {
      for (const module of fit.modules[slotType]) {
        if (!module.offline) {
          const damageMultiplierAttrValue = await fitSimulator.getModifiedAttribute(module.name, 204) || 1;
          if (damageMultiplierAttrValue !== null) {
            totalGlobalDamageBonus += (1 - damageMultiplierAttrValue);
          }
        }
  ...
  /**
   * Convert ESI killmail data to EFT format
   */
  async killmailToEFT(killmailData) {
    // Transform killmail data to ESI fitting format and use the fixed esiToEFT method
    if (!killmailData || !killmailData.victim) {
      throw new Error('Invalid killmail data');
    }

    const victim = killmailData.victim;

    // Create fit name with timestamp
    const killDate = new Date(killmailData.killmail_time).toISOString().split('T')[0];
    const fitName = `Death Fit - ${killDate}`;

    // Transform killmail items to ESI fitting format
    ...
    // Use the fixed esiToEFT method that properly handles loaded charges
    return await this.esiToEFT(esiFitting);
  }

  /**
   * Determine slot type from ESI flag
   */
  getSlotType(flag) {
    // ESI flag to slot mapping
    const flagMap = {
      // High slots
      27: 'high', 28: 'high', 29: 'high', 30: 'high', 31: 'high', 32: 'high', 33: 'high', 34: 'high',
      // Medium slots
      19: 'med', 20: 'med', 21: 'med', 22: 'med', 23: 'med', 24: 'med', 25: 'med', 26: 'med',
      // Low slots
      11: 'low', 12: 'low', 13: 'low', 14: 'low', 15: 'low', 16: 'low', 17: 'low', 18: 'low',
      // Rig slots
    ...
    // Handle string-based flags (e.g., "HiSlot0")
  ...
  /**
   * Check if item is a drone
   */
  async isDrone(itemTypeId) {
    const itemInfo = await this.staticData.getItemInfo(itemTypeId);
    if (!itemInfo) return false;

    return itemInfo.category_id === 18; // Drone category
  }
}
