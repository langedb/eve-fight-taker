...
class FitCalculator {
  ...
  async findOptimalAmmoFromCargo(weaponInfo, fitSimulator) {
    ...
    console.log(`DEBUG: findOptimalAmmoFromCargo for ${weaponInfo.name}, cargo items:`, fit.cargo);
  ...
  async applyModuleStats(module, stats, shipInfo = null, globalDamageBonus = 0, fitSimulator) {
    ...
    console.log(`DEBUG: Module ${module.name} is weapon? ${this.isWeaponModule(moduleInfo)}`);
  ...
  async applyFighterStats(fighter, stats, fitSimulator) {
    ...
    if (!fighterInfo) {
      console.log(`DEBUG: Fighter info not found for: ${fighterName}`);
      return;
    }

    console.log(`DEBUG: Processing fighter: ${fighterName} x${fighterQuantity}, category: ${fighterInfo.category_id}, group: ${fighterInfo.group_id}`);
    ...
    if (emDamage === 0 && explosiveDamage === 0 && kineticDamage === 0 && thermalDamage === 0) {
      ...
      if (fighterDPS) {
        ...
        console.log(`DEBUG: Using fallback DPS values for ${fighterName}: EM:${emDamage}, Explosive:${explosiveDamage}, Kinetic:${kineticDamage}, Thermal:${thermalDamage}`);
      }
    }
    ...
    console.log(`DEBUG: Fighter ${fighterName} damage breakdown: EM:${emDamage}, Explosive:${explosiveDamage}, Kinetic:${kineticDamage}, Thermal:${thermalDamage}`);
    console.log(`DEBUG: Fighter RoF: ${fighterRateOfFire}ms, damage multiplier: ${damageBonusMultiplier}`);
    ...
    const totalFighterDPS = fighterDPSPerUnit * fighterQuantity;

    console.log(`DEBUG: Fighter ${fighterName} total DPS: ${totalFighterDPS} (${fighterDPSPerUnit} per unit x ${fighterQuantity})`);
  ...
  async applyBreacherPodStats(module, stats, fitSimulator) {
    console.log(`DEBUG: Processing breacher pod launcher: ${module.name} with charge: ${module.charge}`);
    
    const podInfo = await this.getItemByName(module.charge);
    if (!podInfo) {
      console.log(`DEBUG: No pod info found for ${module.charge}`);
      return;
    }
    ...
    console.log(`DEBUG: Breacher pod ${module.charge} - Flat: ${flatDamage}HP, Percentage: ${percentageDamage * 100}%, Duration: ${damageDuration}ms`);
    ...
    console.log(`DEBUG: Breacher pod DPS: ${podDPS.toFixed(1)} per pod, ${launcherDPS.toFixed(1)} sustained DPS`);
