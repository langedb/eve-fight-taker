...
class FitCalculator {
  ...
  async calculateEHP(stats, fit, fitSimulator) {
    ...
    const calculateAverageResonance = (resistances) => {
      ...
      for (const type of ['em', 'thermal', 'kinetic', 'explosive']) {
        ...
        averageResonance += damageDistribution[type] * resistances[type];
      }
  ...
  async applyModuleStats(module, stats, _shipInfo = null, _globalDamageBonus = 0, fitSimulator) {
    ...
    const baseCycleTime = await fitSimulator.staticData.getItemInfo(moduleInfo.type_id).then(info => 
      info.attributes?.find(attr => attr.attributeID === 51)?.value || 1000
    );
