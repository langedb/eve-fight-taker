...
class FitCalculator {
  ...
  // Check if item is a module/weapon by group ID
  isModuleItem(itemInfo) {
    const moduleGroups = [
      // Weapon groups
      55, 56,    // Projectile Weapons (AutoCannon, Artillery)
      74, 258,   // Hybrid Weapons (Blaster, Railgun)
      53, 60,    // Energy Weapons (Pulse Laser, Beam Laser)
      507, 508, 509, 510, 511, 771, 812, 1245, // Missile Launchers
      340,       // Probe Launcher
      // Module groups that can have scripts/charges
      212,       // Sensor Booster
      213,       // Tracking Computer
      41, 46, 48, 134, 515, 858, // Propulsion modules (AB, MWD, MJD)
      67,        // Shield Booster
      40,        // Capacitor Booster
      71,        // Armor Repairer
      // Tank modules
      77, 328,   // Shield Extenders
      329,       // Shield Hardeners
      328,       // Armor Plates
      328       // Armor Hardeners
      // Add more module groups as needed
    ];
    return moduleGroups.includes(itemInfo.group_id) || itemInfo.category_id === 7; // Category 7 = Module
  }

  // Check if item is a charge/script/ammo by group ID
  isChargeItem(itemInfo) {
    const chargeGroups = [
      // Weapon ammo/charges
      83, 372, 376,     // Projectile ammo
      85, 373, 1042,    // Hybrid charges
      86, 374, 1043,    // Energy crystals
      384, 385, 387, 388, 389, 390, 391, 392, 393, 394, 395, 396, 655, // Missiles
      479, 482,         // Scanner probes
      // Module scripts
      907,              // Tracking Speed Script
      910,              // Scan Resolution Script, ECCM Script
      // Capacitor charges
      87               // Cap Booster Charges
      // Add more charge/script groups as needed
    ];
    return chargeGroups.includes(itemInfo.group_id) || itemInfo.category_id === 8; // Category 8 = Charge
  }
  ...
  isAmmoCompatibleWithWeapon(weaponInfo, ammoInfo) {
    ...
    // Type compatibility based on group IDs (more reliable than name matching)

    // PROJECTILE WEAPONS - group IDs 55, 76 (weapons) use group IDs 83, 376 (ammo)
    const projectileWeaponGroups = [55, 76]; // Projectile turrets
    const projectileAmmoGroups = [83, 372, 376]; // Projectile Ammo (83), Advanced Autocannon Ammo (372), Advanced Artillery Ammo (376)
    if (projectileWeaponGroups.includes(weaponInfo.group_id) &&
        projectileAmmoGroups.includes(ammoInfo.group_id)) {
      return true;
    }

    // HYBRID WEAPONS - group IDs 74 (weapons) use hybrid ammo groups
    const hybridWeaponGroups = [74]; // Hybrid turrets
    const hybridAmmoGroups = [85, 373, 1042]; // Hybrid charges (85=T1, 373=T2, 1042=Basic)
    if (hybridWeaponGroups.includes(weaponInfo.group_id) &&
        hybridAmmoGroups.includes(ammoInfo.group_id)) return true;

    // LASER WEAPONS - group IDs 394 (weapons) use frequency crystals
    const laserWeaponGroups = [394]; // Energy turrets
    const laserAmmoGroups = [86]; // Frequency crystals
    if (laserWeaponGroups.includes(weaponInfo.group_id) &&
        laserAmmoGroups.includes(ammoInfo.group_id)) return true;

    // MISSILE WEAPONS - various launcher groups use missile ammo groups
    const missileWeaponGroups = [506, 507, 508, 509, 510, 511, 771, 812, 524, 1245]; // Various missile launchers (added 506 for Cruise Launcher, 1245 for Rapid Heavy)
    const missileAmmoGroups = [84, 387, 388, 389, 390, 392, 656, 655]; // Various missiles (added 656 for Advanced Cruise Missile, 655 for Advanced Heavy Missile)
    if (missileWeaponGroups.includes(weaponInfo.group_id) &&
        missileAmmoGroups.includes(ammoInfo.group_id)) return true;
  ...
  async applyModuleStats(module, stats, _shipInfo = null, _globalDamageBonus = 0, fitSimulator) {
    ...
    if (emDamage || thermalDamage || kineticDamage || explosiveDamage) {
      ...
      // console.log(`DEBUG DPS calc: module=${module.name}, cycleTime=${finalCycleTime}ms, dpsFactor=${dpsFactor}, thermalDmg=${thermalDamage}`);
