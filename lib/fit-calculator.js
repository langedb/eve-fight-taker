...
class FitCalculator {
  ...
  isAmmoCompatibleWithWeapon(weaponInfo, ammoInfo) {
    ...
    const projectileAmmoGroups = [83, 376]; // Projectile Ammo, Advanced Artillery Ammo
    ...
    const hybridAmmoGroups = [85]; // Hybrid charges
  ...
  extractWeaponSize(weaponName) {
    ...
    // Medium weapons: 220mm-720mm  
    if (weaponName.includes('220mm') || weaponName.includes('250mm') || 
        weaponName.includes('280mm') || weaponName.includes('350mm') || 
        weaponName.includes('425mm') || weaponName.includes('720mm')) return 'medium';
    
    // Large weapons: 800mm+
    if (weaponName.includes('650mm') || 
  ...
  async killmailToEFT(killmailData) {
    ...
    const shipInfo = await this.staticData.getItemInfo(victim.ship_type_id);
    const shipName = shipInfo ? shipInfo.name : `Ship Type ${victim.ship_type_id}`;
    ...
    if (victim.items) {
      for (const item of victim.items) {
        const itemInfo = await this.staticData.getItemInfo(item.item_type_id);
        if (!itemInfo) continue;

        const itemName = itemInfo.name;
        let slotType = this.getSlotType(item.flag);
        
        // Handle items with charges/ammo
        let chargeName = '';
        if (item.items && item.items.length > 0) {
          const chargeItem = item.items[0];
          const chargeInfo = await this.staticData.getItemInfo(chargeItem.item_type_id);
          if (chargeInfo) {
            chargeName = chargeInfo.name;
          }
        }

        const moduleText = chargeName ? `${itemName}, ${chargeName}` : itemName;
        
        if (slotType) {
          slots[slotType].push(moduleText);
        } else {
          // Handle drones and cargo
          const quantity = item.quantity_destroyed || item.quantity_dropped || 1;
          if (this.isDrone(item.item_type_id)) {
            if (quantity > 1) {
              slots.drones.push(`${itemName} x${quantity}`);
            } else {
              slots.drones.push(itemName);
            }
          } else if (quantity > 1) {
            slots.cargo.push(`${itemName} x${quantity}`);
          }
        }
      }
    }

    // Build EFT format sections
    const sections = ['high', 'med', 'low', 'rig', 'subsystem'];
    for (const section of sections) {
      if (slots[section].length > 0) {
        eftText += slots[section].join('\n') + '\n';
      }
      eftText += '\n';
    }

    // Add drones
    if (slots.drones.length > 0) {
      eftText += slots.drones.join('\n') + '\n\n';
    }

    // Add cargo
    if (slots.cargo.length > 0) {
      eftText += slots.cargo.join('\n') + '\n';
    }
