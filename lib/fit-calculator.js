...
class FitCalculator {
  ...
  async parseEFT(eftText) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (line === '') {
        // Empty line indicates section change only if we've processed modules in current slot
        // EFT format order: Low → Med → High → Rigs → Subsystems → Drones → Cargo (official specification)
        if (currentSection === 'modules' && hasModulesInCurrentSlot) {
      ...
      if (currentSection === 'modules') {
        ...
        if (itemInfo) {
          ...
          else if (isSubsystemSlot && itemInfo.category_id !== 32) {
          ...
          // Check if we're in subsystem slot but couldn't find a subsystem match
