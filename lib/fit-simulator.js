    ...
    for (const slotType of ['high', 'med', 'low']) {
      for (const module of this.fit.modules[slotType]) {
        if (module.charge && !this.chargeAttributes.has(module.charge)) {
          console.log(`FitSimulator: Initializing auto-loaded charge: ${module.charge}`);
    ...
    switch (attributeID) {
      ...
      default:
        ...
        if (attributeValue !== 0 && attributeValue !== 1 &&
          ...
          console.log(`DEBUG: Unknown rig attribute ${attributeID} = ${attributeValue} on ${rigInfo.name}`);
        }
    }
  }
