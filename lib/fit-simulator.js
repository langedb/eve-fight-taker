...
class FitSimulator {
  ...
  async processRigAttribute(attribute, rigInfo, rigModule) {
    ...
    switch (attributeID) {
      // Hydraulic Bay Thrusters detection - check by name since they use effects, not direct attributes
      case 20: // Appears to be a value of 20 on Hydraulic Bay Thrusters
        if (rigInfo.name && rigInfo.name.toLowerCase().includes('hydraulic bay thruster')) {
          await this.applyMissileVelocityBonus(rigInfo, rigModule);
        }
        break;
      ...
      // Targeting range bonuses (attribute 309)
      case 309: // maxTargetRangeBonus
        // Apply targeting range bonus as percentage
  ...
  // Apply missile velocity bonuses based on rig data
  async applyMissileVelocityBonus(rigInfo, rigModule) {
    // Determine velocity bonus from rig tier
    // This is a fallback method since we haven't found the exact attribute yet
    let velocityBonus = 0;
    
    if (rigInfo.name.toLowerCase().includes(' ii')) {
      velocityBonus = 0.15; // T2 version: 15% velocity bonus
    } else if (rigInfo.name.toLowerCase().includes(' i')) {
      velocityBonus = 0.10; // T1 version: 10% velocity bonus  
    }
