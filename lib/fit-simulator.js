...
class FitSimulator {
  ...
  async _calculateModuleBonuses() {
    ...
    // Count BCS and Tracking Computer modules across all slots
    for (const slotType of ['high', 'med', 'low', 'rig', 'subsystem']) {
      for (const module of this.fit.modules[slotType]) {
        const moduleInfo = await this.staticData.searchItemByName(module.name);
        if (moduleInfo) {
  ...
  async _applySingleWeaponBonuses(module, moduleInfo) {
    ...
    if (this.isMissileWeapon(moduleInfo)) {
      ...
      // Apply BCS bonuses to launcher damage multiplier for this weapon type
  ...
  async applyMissileVelocityBonus(attributeValue, rigInfo, rigModule) {
  ...
  async applyMissileFlightTimeBonus(attributeValue, rigInfo, rigModule) {
  ...
  async applyMissileRangeBonus(attributeValue, rigInfo, rigModule) {
  ...
  async applyWeaponRateOfFireBonus(rofMultiplier, rigInfo, rigModule) {
    ...
    if (rofBonus !== 0) {
      ...
      for (const slotType of ['high', 'med', 'low']) {
        for (const slotModule of this.fit.modules[slotType]) {
          if (this.rigAffectsWeapon(rigInfo, slotModule)) {
  ...
  rigAffectsWeapon(rigInfo, weaponModule) {
    ...
    const weaponName = weaponModule.name.toLowerCase();
