class EVEFightTaker {
  constructor() {
    ...
    this.fittingStatsCache = {}; // Cache for fitting stats (DPS, EHP)
  ...
  renderFittingDetails(fittingId, button) {
    ...
    fetch('/api/fleet/fittings/parse', {
      ...
      .then(data => {
        ...
        html += this.generateStatBox('Alpha', Math.round(stats.volley?.total || 0).toLocaleString(), '#ff8787', 'fa-bomb');
        html += this.generateStatBox('EHP', Math.round(stats.ehp?.total || 0).toLocaleString(), '#4ecdc4', 'fa-shield-alt');
        html += this.generateStatBox('Shield', Math.round(stats.ehp?.shield || 0).toLocaleString(), '#5dade2', 'fa-shield');
  ...
  generateStatBox(label, value, color, icon) {
    return `
      <div style="background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); border-left: 3px solid ${color}; border-radius: 6px; padding: 0.75rem; display: flex; flex-direction: column; gap: 0.25rem; height: 70px; justify-content: center; overflow: hidden;">
        <div style="color: rgba(255,255,255,0.6); font-size: 11px; text-transform: uppercase; display: flex; align-items: center; gap: 0.5rem; line-height: 1.2; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
        ...
        <div style="color: white; font-size: 18px; font-weight: bold; line-height: 1.2; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${value}</div>
      </div>
    `;
  }
  ...
  createCompactFittingVisual(parsedFit, _shipTypeId) {
  ...
  async addShipToRole(fittingId, role) {
  ...
  async updateShipQuantity(role, index, quantity) {
  ...
  async removeShipFromRole(role, index) {
  ...
  async getFittingStats(fittingId) {
    // Check cache first
    if (this.fittingStatsCache[fittingId]) {
      return this.fittingStatsCache[fittingId];
    }

    try {
      // Fetch calculated stats from server
      const response = await fetch(`/api/fleet/fittings/${fittingId}/stats`);
      if (!response.ok) {
        console.warn(`Failed to fetch stats for fitting ${fittingId}`);
        return null;
      }
      ...
      // Cache the result
      this.fittingStatsCache[fittingId] = stats;

      return stats;
    } catch (error) {
      console.error(`Error fetching stats for fitting ${fittingId}:`, error);
      return null;
    }
  }
  ...
  async updateFleetStats() {
    ...
    // Collect all unique fitting IDs that need stats
    ...
    Object.values(this.currentFleetComposition).forEach(roleShips => {
      roleShips.forEach(ship => {
        ...
        fittingIds.add(ship.fittingId);
      });
    });

    // Fetch stats for all fittings (in parallel)
    const statsPromises = Array.from(fittingIds).map(id => this.getFittingStats(id));
    await Promise.all(statsPromises);

    // Now calculate totals using cached stats
    Object.values(this.currentFleetComposition).forEach(roleShips => {
      roleShips.forEach(ship => {
        const stats = this.fittingStatsCache[ship.fittingId];
        if (stats) {
          totalDps += stats.dps * ship.quantity;
          totalEhp += stats.ehp * ship.quantity;
        }
      });
    });
    ...
    document.getElementById('fleet-total-dps').textContent = Math.round(totalDps).toLocaleString();
    document.getElementById('fleet-total-ehp').textContent = Math.round(totalEhp).toLocaleString();
  }
  ...
  async saveFleetComposition() {
    ...
    try {
      ...
      if (this.editingFleetId) {
        ...
        // Update fleet name
        const updateResponse = await fetch(`/api/fleet/fleets/${fleetId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: fleetName,
        ...
        // Get current composition to delete existing ships
  ...
  async viewFleet(fleetId) {
    try {
      ...
      // Create working fleet modal with high z-index for visibility
